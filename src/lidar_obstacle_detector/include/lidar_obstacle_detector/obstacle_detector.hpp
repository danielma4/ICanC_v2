...
Box ObstacleDetector<PointT>::axisAlignedBoundingBox(
  ...
  //just gets the box min max points
  pcl::getMinMax3D(*cluster, min_pt, max_pt);

//position represents center, dimension represents total length
...
//pca orientation only accounts for shape of object
//first eigenvalue is the first principal component, the orientation will face in that direction
//making this an inaccurate representation of orientation
template <typename PointT>
Box ObstacleDetector<PointT>::pcaBoundingBox(
  ...
  const Eigen::Quaternionf quaternion(
      eigen_vectors);  // Quaternions are a way to do rotations
...
//we only use rotation if we use principal component analysis
...
void ObstacleDetector<PointT>::obstacleTracking(
  ...
  // problem: sometimes, frames are inconsistent and there are no previous boxes, so tracking isnt perfect
  ...
  if (curr_boxes->empty() || prev_boxes.empty()) {
    ...
    auto connection_matrix =
        ...
        //populates preids and curids as well as creating the adjacency matrix
        //essentially populate the connections with 1 if there is a connection in connection_pairs
...
//find the largest dimension of the two boxes
...
//if the object has moved less than a threshold and the two objects dimensions are less than an iou thresh
...
// this is how we get which box is the same from frame to frame
template <typename PointT>
...
std::vector<std::vector<int>> ObstacleDetector<PointT>::connectionMatrix(
  ...
  //connection pairs is a vector of pairs 
  //remove duplicates of left, right pairs
